...
bool checa_raices(real *c, real *x, int grado, num_tipo_raiz tipo)
{
    ...
    if (tipo)
    {
        ...
        return (!realcmp(LTH, (grado % 2 ? -1 : 1) * c[0] / c[grado]) ?
                CIERTO : FALSO);
    }
    else
    {
        ...
        return (!realcmp(LTH, -c[grado - 1] / c[grado]) ?
                CIERTO : FALSO);
    }
}
...
*
* Notas:
* Se evalúa mediante división sintética todas las raíces. Los resultados
    ...
    if (tipo)
    {
        ...
        for (; conta < grado; conta += 2)
            LTH[conta + 1] = LTH[conta] =
                divsincompleja(grado, c, x[conta], x[conta + 1],
                               b, &yr, &yi);

        return (!realcmp(norma(grado, LTH), 0.) ?
                CIERTO : FALSO);
    }
    else
    {
...
void imprime_raices(real *c, real *x, int grado, num_tipo_raiz tipo)
{
    ...
    if (!grado)
    {
        if (tipo)
            ...
            printf(
                " \tLa ecuación es valida para todos los reales y\n"
                "complejos.\t \n"
            );
        return;
    }
    ...
    if (tipo)
    {
        ...
        for (; conta < grado; conta += 2)
        {
            printf(" \t\tx%d = %#+.19g +%#.19g i\t\t \n",
                   conta + 1, x[conta], x[conta + 1]);
            printf(" \t\tx%d = %#+.19g -%#.19g i\t\t \n",
                   conta + 2, x[conta], x[conta + 1]);
        }
    }
    else
    {
...
/**************************************************************************
* visto_bueno : Nos da el visto bueno para las raíces.
**************************************************************************/
...
* complejos.
***************************************************************************/
real divsincompleja(int grado, real *c, real x, real y,
                    real *b, real *yr, real *yi)
{
    int conta = grado - 2;
    ...
    if (grado > 1)
    {
        ...
        (grado > 2) ?
            (b[conta] = c[grado - 1] - u * b[grado - 2]) :
            (r = c[grado - 1] - u * b[grado - 2]);

        for (conta = grado - 4; conta >= 0; conta--)
            b[conta] = c[conta + 2] - u * b[conta + 1]
                       - v * b[conta + 2];
...
/**************************************************************************
* norma : Calcula la norma natural de un vector.
**************************************************************************/
...
void error1(void)
{
    perror("\aFaltan los coeficientes de la ecuacion\n");
    abortar();
}

void errorn(void)
{
    perror("\aDemasiados coeficientes de la ecuacion\n");
    abortar();
}

void error_opcion(void)
{
    perror("\aOpcion invalida\n");
    abortar();
}
